...
export function extractFromAsar(sourcePath: string, destPath: string) {
    // Use process.resourcesPath for production and app.getAppPath() for development
    const basePath = app.isPackaged ? process.resourcesPath : app.getAppPath();
    const resolvedSourcePath = path.isAbsolute(sourcePath) ? sourcePath : path.join(basePath, sourcePath);
    const resolvedDestPath = path.isAbsolute(destPath) ? destPath : path.join(basePath, destPath);
